...
class OffsetStorage {
    ...
    /**
     * Gets the desired number of indent for a token
     * @param {Token} token The token
     * @returns {number} The number of desired indents for the token
     */
    getDesiredIndents(token) {
        if (!this._desiredIndentCache.has(token)) {
            ...
            if (this._ignoredTokens.has(token)) {
                ...
                this._desiredIndentCache.set(
                    token,
                    this._tokenInfo.getTokenIndent(token).length / this._indentType.length
                );
                ...
                this._desiredIndentCache.set(
                    token,
                    this.getDesiredIndents(this._tokenInfo.getFirstTokenOfLine(firstToken))
                );
            } else {
                ...
                this._desiredIndentCache.set(token, offset + (offsetInfo.from ? this.getDesiredIndents(offsetInfo.from) : 0));
            }
        }
...
module.exports = {
    ...
    create(context) {
         ...
         * @param {number} desiredIndents Desired number of indentations
         * @returns {boolean} `true` if the token's indentation is correct
         */
        function validateTokenIndent(token, desiredIndents) {
            const indentation = tokenInfo.getTokenIndent(token);
            const expectedChar = indentType === "space" ? " " : "\t";

            return indentation === expectedChar.repeat(desiredIndents * indentSize) ||
            ...
            ignoredNodeListeners,
            {
                ...
                "Program:exit"() {
                    ...
                    sourceCode.lines.forEach((line, lineIndex) => {
                        ...
                        if (validateTokenIndent(firstTokenOfLine, offsets.getDesiredIndents(firstTokenOfLine))) {
                            return;
                        }
                        ...
                        if (astUtils.isCommentToken(firstTokenOfLine)) {
                            ...
                            if (
                                tokenBefore && validateTokenIndent(firstTokenOfLine, offsets.getDesiredIndents(tokenBefore)) ||
                                tokenAfter && validateTokenIndent(firstTokenOfLine, offsets.getDesiredIndents(tokenAfter))
                            ) {
                                return;
                            }
                        }
                        ...
                        report(firstTokenOfLine, offsets.getDesiredIndents(firstTokenOfLine));
                    });
                }
            }
        );
